...
class FUser {
    ...
    init(objectId: String, pushId: String?, createdAt: Date, updatedAt: Date, email: String, firstname: String, lastname: String, avatar: String = "", loginMethod: String, phoneNumber: String, city: String, country: String) {
    ...
    init(dictionary: NSDictionary) {
        ...
        if let updateded = dictionary[kUPDATEDAT] {
        ...
        if let onl = dictionary[kISONLINE] {
            isOnline = onl as! Bool
        } else {
            isOnline = false
        }
        if let phone = dictionary[kPHONE] {
        ...
        if let cont = dictionary[kCONTACT] {
            contacts = cont as! [String]
        } else {
            contacts = []
        }
        if let block = dictionary[kBLOCKEDUSERID] {
            blockedUsers = block as! [String]
        } else {
            blockedUsers = []
        }

        if let lgm = dictionary[kLOGINMETHOD] {
            loginMethod = lgm as! String
        } else {
            loginMethod = ""
        }
        if let cit = dictionary[kCITY] {
            city = cit as! String
        } else {
            city = ""
        }
        if let count = dictionary[kCOUNTRY] {
    ...
    class func currentUser () -> FUser? {
        ...
        if Auth.auth().currentUser != nil {
            ...
            if let dictionary = UserDefaults.standard.object(forKey: kCURRENTUSER) {
                ...
                return FUser.init(dictionary: dictionary as! NSDictionary)
            }
        }
    ...
    class func registerUserWith(email: String, password: String, firstName: String, lastName: String, avatar: String = "", completion: @escaping (_ error: Error?) -> Void ) {
        ...
        Auth.auth().createUser(withEmail: email, password: password, completion: { (firuser, error) in
            ...
            let fUser = FUser(objectId: firuser!.user.uid, pushId: "", createdAt: Date(), updatedAt: Date(), email: firuser!.user.email!, firstname: firstName, lastname: lastName, avatar: avatar, loginMethod: kEMAIL, phoneNumber: "", city: "", country: "")
        ...
        Auth.auth().signInAndRetrieveData(with: credential) { (firuser, error) in
            ...
            fetchCurrentUserFromFirestore(userId: firuser!.user.uid, completion: { (user) in
                ...
                if user != nil && user!.firstname != "" {
                    ...
                    let fUser = FUser(objectId: firuser!.user.uid, pushId: "", createdAt: Date(), updatedAt: Date(), email: "", firstname: "", lastname: "", avatar: "", loginMethod: kPHONE, phoneNumber: firuser!.user.phoneNumber!, city: "", country: "")
